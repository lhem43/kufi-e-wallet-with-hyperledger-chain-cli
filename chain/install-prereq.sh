...
# Also: restores Go dependencies for CLI + chaincode modules
...
# ── 6. Build KufiChain ───────────────────────────────────────────────
restore_go_modules() {
    info "Downloading Go dependencies (CLI + chaincode modules)..."

    local SCRIPT_DIR
    SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
    local modules=(
        "$SCRIPT_DIR"
        "$SCRIPT_DIR/chaincode/payment"
        "$SCRIPT_DIR/chaincode/governance"
    )

    local mod label
    for mod in "${modules[@]}"; do
        if [ ! -f "$mod/go.mod" ]; then
            warn "  Skipped (go.mod not found): $mod"
            continue
        fi

        if [ "$mod" = "$SCRIPT_DIR" ]; then
            label="chain (root module)"
        else
            label="${mod#$SCRIPT_DIR/}"
        fi

        info "  Resolving module: $label"
        (cd "$mod" && go mod download && go mod verify)
        ok "  Dependencies ready: $label"
    done
}
